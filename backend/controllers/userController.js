...
const verifyRazorpay = async (req, res) => {
  try {
    ...
    console.log("razorpayId" , razorpay_order_id);
...
const emergencyAlert = async (req, res) => {
  try {
    ...
    // 2. Find first available doctor
    const doctor = await doctorModel.findOne({ available: true });
    if (!doctor) {
      return res.status(404).json({ success: false, message: 'No available doctors' });
    }

    // 3. (Simulated) Send alert - you can add SMS logic here
    console.log(`🚨 Emergency Alert! Notify Doctor: ${doctor.name} (${doctor.phone})`);
    console.log(`User in Emergency: ${user.name} (${user.phone})`);

    // Return response
    return res.status(200).json({
      success: true,
      message: `Emergency alert sent to Dr. ${doctor.name}`,
    });
  } catch (error) {
